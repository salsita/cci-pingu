...
const startScript = () => {
  ...
  try {
    const script = spawn(scriptFile, [ config._target ], { cwd: path.dirname(scriptFile) });
    script.stdout.on('data', (data) => console.info('Script STDOUT:\n' + data.toString()));
    script.stderr.on('data', (data) => console.warn('Script STDERR\n:' + data.toString()));
    script.on('error', (err) => {
...
module.run = (_options = null) => {
  ...
  request
    .get('https://circleci.com' + apiPath)
    .query({ 'circle-token': config.token, filter: 'successful', limit: 1 })
    .set('Accept', 'application/json')
    .timeout(config.timeout * 1000)
